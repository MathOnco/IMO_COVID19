                                ...
                                dateStart = as.Date('2019-11-01'),POP = wuhanpop,numWeekStagger=c(2,4,6),pInfected=0.0002,durInf = 7,contacts_china=contacts)
{
  ...
  z=1
  while(ind>0){
    infected.groups[z] = sample(3:13, 1) # Assuming group in contact is within travel age range (3-14 vs 1 (kids) to 16 (geriatric))
    num.Infected[z] <- sample(ind, 1)
    ind = abs(pInfected-sum(num.Infected))
    z=z+1
  }
  
  E[1,] = 0 
  I[1,infected.groups] = num.Infected # 100 # Assign 100 infected person in each age group (TODO RELAX?)
  R[1,] = 0 
                                    ...
                                    dateStart = as.Date('2019-11-01'),POP = wuhanpop,numWeekStagger=c(2,4,6),pInfected=0.0002,durInf = 7,contacts_china=contacts)
{
  ...
  E[1,] = 0 
  Ic[1,infected.groups] = num.Infected # 100 # Assign 100 infected person in each age group (TODO RELAX?)
  Isc[1,] = 0 
  R[1,] = 0 
